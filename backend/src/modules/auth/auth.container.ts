...
import { StudentDirectoryRepository } from "#@/modules/auth/repositories/student_directory.repository.js"
...
class AuthContainer {
    public keystoreRepo = new KeyStoreRepository(supabaseDB);
    public studentDirectoryRepo = new StudentDirectoryRepository(supabaseDB);

    public keystoreService = new KeystoreService(this.keystoreRepo);

    public authService = new AuthService(this.keystoreService);
    public authStrategy = new MicrosoftAuthStrategy(userFacade, this.studentDirectoryRepo);
    public authGoogleStrategyTest = new GoogleAuthStrategy(userFacade, this.studentDirectoryRepo);
