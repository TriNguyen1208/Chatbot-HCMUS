...
class Chatbot:
    def __init__(self, collection_name: str = COLLECTION_NAME):
        ...
        self.db.client.create_payload_index(
    ...
    def _merge_adjacent_chunks(search_results: List[Dict[Any, Any]], max_chunk_dist: int = 1) -> List[Dict[str, Any]]:
        """
    ...
    def answer(self, user_query: str) -> Tuple[str, List]: 
        if user_query.strip().lower() in ['quit', 'exit']:
            return None, None

        raw_results = self.db.hybrid_search_with_rerank(COLLECTION_NAME, user_query, prefetch_limit=50, fusion_limit=30, top_k=15)
        raw_results = [item.payload for item in raw_results]
        if (raw_results == []):
            print("> Trả lời: Không đủ dữ kiện trả lời!")
            return None, None
        
        context_blocks = self.db.build_context_blocks(COLLECTION_NAME, raw_results, extra_chunks=2)
        ...
        evidence = [context_blocks[i]["content"] for i in result.source_indices]
